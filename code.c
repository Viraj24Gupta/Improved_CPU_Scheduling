...
void innovative() {
    ...
    while (1) {
        ...
        if (tq == 0) printf("All the processes has been executed\n");
        for (i = 0, count = 0; i < n; i++) {
            if (cc == 0) { burst[i] = st[i];
            }
            temp = tq;
            if (st[i] == 0) {
                count++;
    ...
    for (int i = 0; i < n; i++) printf("%d ", turn[i]);
    ...
    do {
        ...
        switch (c) {
            ...
            case 4:
                ...
                break;
            default:
                printf("Please enter choice from 1 to 4 only\n");
                break;
        }
